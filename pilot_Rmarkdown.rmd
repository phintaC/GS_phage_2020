...
```{r, eval=TRUE}
# Read input 
data.bc = read.table("datasets/virus_id_relabundance.phages.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)

# Create tables wiht regional mean abundance
# Group data by income and find means for each phage
data.grouped <- aggregate(data[,3:ncol(data)], list(data$region), mean)

levels(data.bc$region) <- c("AF","AS","EU","ME","NA","OC","SA")

# Exclude non-numerical column; region
myvars.bc <- names(data.bc) %in% c("region") 
data_phage.bc <- data.bc[!myvars.bc]

# Transpose dataframe
phage.bc <- data.frame(data_phage.bc[,3:ncol(data_phage.bc)], row.names=data_phage.bc[,1], check.names=FALSE) 


#Calculate dissimilarity matrix
phage2.bc <- vegdist(decostand(phage.bc,method = "hellinger"), dist="bray")
# This is not a true distance metric, but rather a dissimilarity

# dataframe
label.bc <- data.frame(data_phage.bc[,2:ncol(data_phage.bc)], row.names=data_phage.bc[,1], check.names=FALSE) 

# Get region label from dataframe
metadata.bc = data.frame(sample=rownames(label.bc), label.bc, group=c(rep("AF",13), rep("AS",9), rep("EU",31), rep("ME",3), rep("NA",14), rep("OC",3), rep("SA",5)))

# PCoA plot with Ellipse (Ellipses are by default 1 SD around group medians)
# Beta disper reduces the original (non-Euclidean) distances to prinicpal coordinates 
?betadisper
betadisp.all.bc <- betadisper(phage2.bc, data.bc$region, add=F)

eigenperc.bc <- round(as.vector(eigenvals(betadisp.all.bc) / sum(eigenvals(betadisp.all.bc)[eigenvals(betadisp.all.bc)>0]))*100, digits=2)

# Number of regions
num_var.bc = length(unique(data.bc$region))

# Setup output and plot to file
pdf('plots/final/2016_PCoA_NCBI.phages.pdf',onefile=TRUE)

plot(betadisp.all.bc, 
     ellipse = T, 
     hull = F, 
     col=c("#E41A1C", "#377EB8", "#4DAF4A", "#984EA3", "#FF7F00", "#FFFF33", "#A65628"), 
     pch=rep(16,num_var.bc), 
     lty=1, 
     lwd=2, 
     segments=T, 
     ...
     label=T, 
     cex=1.5, 
     main="Distribution of NCBI - phages\n2016", 
     seg.lty=2, 
     seg.lwd=1,
     xlab=paste("PCoA1 (", eigenperc.bc[1] , "% variation)"), 
     ylab=paste("PCoA2 (", eigenperc.bc[2] , "% variation)")) 
dev.off()
```
...
```{r, eval=TRUE}
# Read input 
data = read.table("datasets/virus_id_relabundance.virus.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
# Exclude non-numerical column; region
...
# Get region label from dataframe
metadata = data.frame(sample=rownames(label), label, group=c(rep("Africa",13), rep("Asia",9), rep("Europe",31), rep("Middle East",3), rep("North America",14), rep("Oceania",3), rep("South America",5))) 
...
```{r, eval=TRUE}
# Read input 
data = read.table("datasets/bacteria_species_name_relabundance.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)
...
# Get region label from dataframe
metadata = data.frame(sample=rownames(label),
                      label, 
                      group=c(rep("Africa",12),
                              rep("Asia",10), 
                              rep("Europe",31),
                              rep("Middle East",3),
                              rep("North America",14),
                              rep("Oceania",3), 
                              rep("South America",5))) 
...
```{r , eval=TRUE}
# Read input 
data = read.table("datasets/imgvr_fragments/imgvr_id_relabundance.phages.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
# Exclude non-numerical column; region
myvars <- names(data) %in% c("region") 
data_phage <- data[!myvars]
...
# Get region label from dataframe
metadata = data.frame(sample=rownames(label), label, group=c(rep("Africa",12), 
                                                             rep("Asia",10), 
                                                             rep("Europe",31), 
                                                             rep("Middle East",3), 
                                                             rep("North America",14),
...
```{r, eval=TRUE}
# Read input 
data = read.table("datasets/imgvr_genomes/imgvr_id_relabundance.phages.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
...
# Get region label from dataframe
metadata = data.frame(sample=rownames(label), label, group=c(rep("Africa",12), rep("Asia",10), rep("Europe",31), rep("Middle East",3), rep("North America",14), rep("Oceania",3), rep("South America",5))) 
...
```{r, eval=TRUE}
# Read input 
data = read.table("datasets/imgvr_fragments/imgvr_id_relabundance.virus.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
...
```{r, eval=TRUE}
# Read input 
data = read.table("datasets/custom_id_relabundance.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
# Exclude non-numerical column; region
myvars <- names(data) %in% c("region") 
data_phage <- data[!myvars]
...
```{r,eval=TRUE}
# Read input 
data = read.table("datasets/pcoa/phages.concatenated.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)

# Update regin annotation
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
...
PCoA - concatenated phages - IMG/VR genomes
```{r,eval=TRUE}
# Read input 
data = read.table("datasets/pcoa/phages.concatenated.ncbi_custom_imgvrGenomes.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)
...
```{r, eval=TRUE}
# Read input 
data = read.table("datasets/pcoa/virus.concatenated.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)

# Update regional annotation 
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
...
```{r, eval=TRUE}
# Read data
data = read.table("datasets/resistance/resfinder_genes_FPKM.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)  

# Update lavel annotation 
levels(data$region) <- c("AF","AS","EU","ME","NA","OC","SA")
...
```{r, eval=TRUE}
# Read data
data = read.table("datasets/resistance/functionalResistance_genes_FPKM.tsv", sep="\t", dec = '.',header=TRUE, check.names = F)  
