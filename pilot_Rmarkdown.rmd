...
# Calculate homoscedasticity
betadisp.all <- betadisper(data.dist, data$region, add=F)

# Calculate principal coordinate variances
...
# Plot the data
pdf("plots/final/2016_ternary_ncbi.phages.full.pdf")
...
# Plot the data
pdf("plots/final/2016_ternary_imgvr.genomes.full.pdf")
ggtern(data.grouped, aes(x = high, y = middle, z = low)) +
  # stat_confidence_tern(geom="polygon", aes(fill=..level..), color="white") +
  geom_point_swap(fill = "gray50", stat = "identity", alpha=0.4,shape=16, size = 5) +
  geom_density_tern(geom="polygon", bdl=0.001) +
...
# Plot the data
pdf("plots/final/2016_ternary_huge_phages.full.pdf")
ggtern(data.grouped, aes(x = high, y = middle, z = low)) +
  # stat_confidence_tern(geom="polygon", aes(fill=..level..), color="white") +
  geom_point_swap(fill = "gray50", stat = "identity", alpha=0.4,shape=16, size = 5) +
  geom_density_tern(geom="polygon", bdl=0.001) +
...
Ternary plot - huge phages - relabundances
```{r, eval=TRUE}
data <- read.delim("datasets/income/custom_hosts.tsv",header=1,row=1)
...
# Plot the data
pdf("plots/final/2016_ternary_huge_phages.hosts.full.pdf")
ggtern(data.grouped, aes(x = high, y = middle, z = low)) +
  # stat_confidence_tern(geom="polygon", aes(fill=..level..), color="white") +
  geom_point_swap(fill = "gray50", stat = "identity", alpha=0.4,shape=16, size = 5) +
  geom_density_tern(geom="polygon", bdl=0.001) +
   # Labels
  labs(title="Phage distribution stratified by income class\nHuge phage hosts 2016") +
  xlab("High-income") +
  ylab("Middle-income") +
  zlab("Low-income") +
  geom_text(aes(label=phage_num),size=2) +
...
write.table(data.grouped, "custom_tern.hosts.xlsx",sep="\t",row.names=T,col.names=T)
...
breaklist <- quantile_breaks(t(data.num), n = 100)
...
# Heatmap plotting
pdf("2016_pheatmap_NCBI_phages.top15_sample.raw.100.pdf", height = 32, width = 16)
...
Heatmap top 15 per sample - NCBI bacteria genus
```{r}
...
p3 <- ggplot(J, aes(x=region,y=J$diversity,fill=region)) +
  (d <- geom_boxplot(show.legend = F, outlier.shape = NA)) +
  geom_jitter(show.legend = F,shape=21) +
